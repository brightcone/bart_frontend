...
//     };
    ...
    const TicketCard = ({ ticketNo, link, assignedTo, time, urgency }) => {
 
const date = new Date();

// Get the correct month index and day
const day = String(date.getDate()).padStart(2, '0'); // Day of the month
const monthIndex = date.getMonth(); // Get the correct month index (0-based)
...
// Format the time
const formattedTime = `${monthName} ${day}, ${date.toLocaleString('en-US', {
    hour: 'numeric',
    minute: 'numeric',
    hour12: true,
}).replace(' ', '')}`;
        ...
        return (
            <div className="ticket-card" onClick={() => window.open(link, '_blank')}>
                <div className="ticket-header">
                    <div className="ticket-time-urgency">
                        {urgency && <span className="ticket-urgency">• {urgency}</span>}
                        <span className="ticket-time" style={{ marginRight: '8px' }}>
  {formattedTime} <span style={{ marginLeft: '7px', color: '#ff6666' }}>. Urgent</span></span>
                ...
                <div className="ticket-details">
                    <div>
                    <div className="ticket-label" style={{ fontSize: '14px' }}>Ticket No.</div>

                      <div className="ticket-value">{ticketNo}</div>
                    </div>
                    <div>
                    <div className="ticket-label" style={{ fontSize: '14px' }}>Assigned to</div>  
                                            <div className="ticket-assigned">
                                            <img src={ChatLogo} alt="Profile" className="assigned-profile" />
